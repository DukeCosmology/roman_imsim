...
def _snell_refract_unit(p, n_hat, n_old, n_new):
    ...
    n_hat : array_like, shape (2,) or (..., 2)
        Unit outward surface normal (pointing away from the centre of curvature).
...
def _sphere_sag(q, R):
    ...
    R : float
        Radius of curvature, metres.  Positive means the centre of curvature
...
class RomanFilterRefraction(galsim.PhotonOp):
    ...
    SCA_pos : galsim.PositionD, optional
        Pixel position on the SCA.  Defaults to detector centre (2044, 2044).
    """
    # -----------------------------------------------------------------------
    ...
    def applyTo(self, photon_array, local_wcs=None, rng=None):
        ...
        if not photon_array.hasAllocatedWavelengths():
            raise RuntimeError(
                "No wavelengths on PhotonArray. Add a WavelengthSampler first."
            )
    ...
    def _get_lateral_shifts(self, lam1, lam2=None):
        ...
        common = dict(
            nfunc=self.n, lam=lam1, lam_ref=lam2,
            t=FILTER_THICKNESS, R1=R1, R2=R2,
            s_pupil_to_S1=PUPIL_TO_S1, L_S2_to_fpa=S2_TO_FPA,
        )
        # get x and y shifts
    ...
    def buildPhotonOp(self, config, base, logger):
        if 'bandpass' not in base:
            raise galsim.GalSimConfigError(
                "bandpass is required in base config for RomanFilterRefraction"
            )
        if 'image' not in base or 'SCA' not in base['image']:
            raise galsim.GalSimConfigError(
                "SCA must be set in image config for RomanFilterRefraction"
            )
        if 'image_pos' not in base:
            raise galsim.GalSimConfigError(
                "image_pos must be set in base config for RomanFilterRefraction"
            )
