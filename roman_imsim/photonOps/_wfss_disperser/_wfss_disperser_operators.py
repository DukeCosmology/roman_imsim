...
class WFSSSDisperser(PhotonOp):
    ...
    def _disperse(self, x0, y0, lam, sca, order="1"):
        ...
        x_pow = (
            xfpa_deg[:, np.newaxis, np.newaxis, np.newaxis]
            ** np.arange(6)[np.newaxis, np.newaxis, :, np.newaxis]
        )
        y_pow = (
            yfpa_deg[:, np.newaxis, np.newaxis, np.newaxis]
            ** np.arange(6)[np.newaxis, np.newaxis, np.newaxis, :]
        )
    ...
    def applyTo(self, photon_array, local_wcs=None, rng=None):
        ...
        x_pix, y_pix = self.snpit_disperser.disperse(
            photon_array.x, photon_array.y, w, self.sca, self.order, pairwise=True
        )
