...
class nonlinearity(roman_effects):
    """
    ...
    def lab_model(self, image):
        if self.sca_filepath is None:
            self.logger.warning("No non-linearity data file provided; no non-linearity effect will be applied.")
            return image
        ...
        image.array[:,:] -= self.df['CNL'][0,:,:][0] * image.array**2 +\
