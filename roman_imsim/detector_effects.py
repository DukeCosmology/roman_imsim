...
class detector_effects(object):
    """
    Class to simulate non-idealities and noise of roman detector images.
    """
    def __init__(self, params, visit, sca, filter, logger, rng, rng_iter, sca_filepath=None):
        self.sca = sca
        self.filter = filter
        if sca_filepath:
            self.df = fio.FITS(os.path.join(sca_filepath, sca_number_to_file[self.sca]))
    ...
    def setup_sky(self, im, pointing=None, force_cvz=False, stray_light=False, thermal_background=False):
        """
        Setup sky
        ...
        if pointing is None:
            pointing = self.pointing
        if self.df is None:
            self.dark_current_ = roman.dark_current * pointing.exptime
        else:
            self.dark_current_ = roman.dark_current * pointing.exptime + self.df['DARK'][:, :].flatten() * pointing.exptime
        if self.df is None:
    ...
    def recip_failure(self, im, exptime=None, alpha=roman.reciprocity_alpha, base_flux=1.0):
        """
    ...
    def dark_current(self, im, exptime=None):
        """
    ...
    def add_persistence(self, im, pointing=None):
        """
        ...
        if self.df is None:
            ...
            for p in p_pers:
                dt = (pointing.date - p.date).total_seconds() - pointing.exptime/2 ##avg time since end of exposures
            ...
            for p in p_pers:
                dt = (pointing.date - p.date).total_seconds() - pointing.exptime/2 ##avg time since end of exposures
                fac_dt = (pointing.exptime/2.) / dt  ##linear time dependence (approximate until we get t1 and Delat t of the data)
                self.params['output']['file_name']['items'] = [p.filter, p.visit, p.sca]
                imfilename = ParseValue(self.params['output'], 'file_name', self.params, str)[0]
                fn = os.path.join(self.params['output']['dir'], imfilename)
