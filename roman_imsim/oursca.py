...
class RomanSCAImageBuilder(ScatteredImageBuilder):
    ...
    def writeASDF(self, path, include_raw_header=False):
        """
        ...
        if self.fitsheader is not None:
            ...
            tree['meta']['ra_pointing'] = self.fitsheader['RA_TARG'] #not sure if this is the same as in wcs
            tree['meta']['dec_pointing'] = self.fitsheader['DEC_TARG']
            tree['meta']['zptmag'] = 2.5 * np.log10(self.exptime * roman.collecting_area)
